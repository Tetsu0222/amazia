...
フルスタック EC 基盤を実運用レベルで構築・運用するためのシミュレーションプロジェクトです。
AI と協働しながら、業務レベルの EC プラットフォームを 設計 → 実装 → デプロイ → 運用 まで一貫して作り上げることを目的としています。
...
業務の現場でも TDD や CI への意向は上席含めて存在していましたが、それらを業務の中で実装しきるには、プロジェクトを取り巻く前提条件をいくつも乗り越える必要がありました。そして何より、**ビジネスの場で「実験」をモチベーションに業務にあたることは、自分の職業倫理として避けたい**。実験は実験の場で、業務は業務の場で。そう切り分けた上で、自分の仮説を最後まで検証しきれる場として個人プロジェクトという形を選びました。
...
ソフトウェア開発が「AI に勝てない領域」になるかどうかは、**いかにして開発タスクを閉じた文脈として構造化できるか** にかかっている。
