...
- **Phase 1〜16**：✅ 完了（会員画面・管理画面・商品 CRUD・Excel 一括登録・商品マスタ・在庫/価格管理・認証・ワークフロー・購入・配送）
- **Phase 17〜20**：🟡 着手中（バッチ処理・問い合わせ・お知らせ・**ドキュメント整理**）
- **Phase X-1〜X-8**：本番運用で発生した課題への対応フェーズ（HTTPS 化・メモリ最適化・デプロイ後ヘルスチェック・AI 協働アンチパターン整備など。X-2〜X-4, X-6, X-7 完了済み）
...
本プロジェクトの本番 AWS 環境は Landing Zone 最小構成レベル（レベル1相当） をスコープとしています。Multi-AZ 冗長化・WAF・専用 VPC 設計といった LAC レベル2以上の項目は、学習目的・無料枠維持の観点から意図的に範囲外としています（2026-05 時点）。
