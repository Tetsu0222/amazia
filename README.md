...
Amazia は、商品管理・在庫管理・EC フロントを一体化した、**3 システム構成の EC 基盤**です。

会員向けサイト（Market）、管理画面（Console）、在庫・バッチ処理を担うコアシステム（Core）の 3 つで構成され、それぞれが独立した技術スタック（React / Vue / Java / PHP）で実装されています。
