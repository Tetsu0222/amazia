...
-- ============================================================================
-- 商品マスタの product_statuses は本番にも投入される基盤マスタ
-- （Product Entity の status_code カラムが参照する。空だと商品作成不可）
-- ============================================================================
...
-- ============================================================================
-- ローカル開発用のサンプル商品 seed は本ファイルに含めない。
-- ローカル限定の seed は data-local.sql に切り出し、`local` プロファイル時のみ
-- 追加で読み込む（application-local.properties で spring.sql.init.data-locations を上書き）。
-- 本番 EC2 にデプロイされる data.sql には基盤マスタのみが入る。
-- ============================================================================
