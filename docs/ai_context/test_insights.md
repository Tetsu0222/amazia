...
### Ant Design Vue ラッパーコンポーネントの落とし穴（047 起因）
- `a-popconfirm` / `a-popover` / `a-tooltip` などのラッパーは実 DOM を持たないため、それ自体に `@click.stop` を付けても**スロット内の要素まで伝播停止が効かない**。`@click.stop` は内側の `<a-button>` など実 DOM を持つ要素に付ける
- テーブルの末尾「操作列」で `a-popconfirm` を使うときは `placement` を必ず明示する。デフォルト（`top`）はビューポート右端で本体が画面外に押し出され、`.ant-popover-buttons` のボタンが折り返して縦積みになるレイアウト崩れが起きる。右寄せ列なら `topRight` / `bottomRight` を指定する
- `expand-row-by-click` を有効にしたテーブルでは、行内のすべての操作要素（編集／SKU管理／削除など）に `@click.stop` を付けないと行展開と操作が混在する

### テスト観点（Ant Design Vue UI）
- [ ] 操作列の Popconfirm に `placement` 指定があるか（画面端の崩れ防止）
- [ ] 行内ボタンに `@click.stop` が付いているか（行展開との混線防止）
- [ ] **これらは jsdom（Vitest/Jest）では検出不能なため、ブラウザ目視と PR スクリーンショットでチェックする**
...
### `@UniqueConstraint` を持つ Entity のテストヘルパー設計（050 起因）

H2 + `ddl-auto=create-drop` は Entity の `@UniqueConstraint` を忠実に DDL 化する。テストヘルパーで制約カラムをハードコードしたまま同 Entity を複数行作ると、テストの2件目で UNIQUE 違反になる。これは 027/037/038/044/045/049 と同じ「H2/本番 MySQL 乖離」系統の **逆向き顕在化**（H2 が Entity 通りに作るからこそ落ちる）。

- ヘルパーで一意キーが必要な列は **毎回ユニーク化**する（`System.nanoTime()` サフィックス、引数で受け取る、など）。`skuCode` だけ一意化して `color`/`size` を固定すると `(product_id, color, size)` UNIQUE で衝突する（050 の事故）
- 既存テストで「色違い・サイズ違いで複数行を作る前例」があるかを `setColor\(|new <Entity>\(` 系の grep で先に確認する。前例があれば運用パターンに合わせる
- Entity 側を変更したら `@UniqueConstraint` / `uniqueConstraints = ...` の宣言を grep し、影響テストヘルパーを再点検

#### テスト観点（追加）
- [ ] `@UniqueConstraint` を持つ Entity を複数行作るテストヘルパーは、制約カラムを必ずユニーク化または引数化しているか
...
| **テスト DB 初期化方式と本番 DB 初期化方式の差異**（H2 + ddl-auto では通るが本番 MySQL + schema.sql では通らない／逆方向に H2 が Entity 制約を忠実に作るためテストヘルパーが衝突するケースもある） | 027, 037, 038, 044, 045, 049, 050 |
