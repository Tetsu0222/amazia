...
AP-005（046→048）・AP-006（020/030/041）・AP-001（044→045→049）・AP-008（003/008/023/025/026）・AP-009（051 派生①〜③）には
...
本 AP-001〜009 に直接含まれていない。

これらは「人間側の設計判断・前提共有が不十分なまま AI に渡した結果」として共通根を持つ。
今後類似事例が積み上がった段階で、新規 AP として「人間側の前提整備不足」を独立追加するかを検討する。
...
**症状：**
H2 共有 DB（`@SpringBootTest` のキャッシュ機構で全テストクラスが同一インスタンスを共有）で件数アサーションが失敗。
`ApplyScheduledPricesJobTest.APP_3` の単発失敗から始まり、1 クラスずつ `@Transactional` を付与する対症療法を 4 段階繰り返した結果、
最終的に `@SpringBootTest` 全 46 クラスへ一括付与し **6 クラスは設計上 revert**（マルチスレッド検証 / REQUIRES_NEW 検証 / テーブル間アーカイブ）。
`@Transactional(REQUIRES_NEW)` 経由の書き込み（`FaultInjectionLogger` / `BatchAlertNotifier` / `BatchExecutionRecorder`）はロールバックを貫通するため、
件数アサーションを行う Repository テストへ自衛コード（`@BeforeEach cleanupPriorXxx`）が増殖し、
**「テストを通すこと」が「不具合を検知すること」を浸食**する状況に至った。

**AI 側の判断ミス：**
- 派生① で `ApplyScheduledPricesJobTest.APP_3` を見て当該 1 クラスへ `@Transactional` 付与で完了とし、同型汚染が他クラスにあるか grep しなかった
- 派生② で `FaultInjectionLogRepositoryTest` / `NotificationSubscriptionRepositoryTest` を直すとき、また 1 クラスずつ対応して終わらせた
- 派生③前半で全 46 クラスへ一括 `@Transactional` 付与した際、Javadoc で「クラスレベル `@Transactional` を付けない」と書かれていたテスト（マルチスレッド検証等）の検証意図を踏み潰した
- 派生③後半で REQUIRES_NEW 貫通レコードに対し「自衛コードを追加して件数アサーションを通す」方向に流れ、構造的な分離（cleanup.sql + `@Sql`）を検討しなかった
- 派生③追加で `INV_2` の before スナップショット計算ミスが random 順序で初めて顕在化したのに、「自テスト所有 ID でフィルタ」する設計規約として整理しなかった

**人間側の予防機会：**
- 派生① の段階で「同型汚染が他クラスに無いか」を AI に grep させ、横展開確認のゲートを設ける
- 全クラス一括 `@Transactional` のような「広い範囲を機械的に書き換える PR」をレビューで止め、クラス Javadoc の検証意図を踏み潰していないか確認させる
- 件数アサーションの自衛コードが増えてきた段階で「症状療法の限界」と判断し、構造的対策（cleanup.sql / Testcontainers）の検討フェーズを切る
- random 順序での 5 回連続 PASS をローカル必須化し、push 直後の CI で踏むパターンを事前検出

**検知タイミング：**
CI の `mvn test` 全体実行で初めて検知。**単体実行 / パッケージ単体実行では PASS** という最も気付きにくい形で顕在化する（051）。

**事前ガード：**
- `test_insights.md` カテゴリ 7-2 に「`@Transactional` を付ける/付けない判断軸」「件数アサーション規約」「スナップショット差分規約」を明文化（phaseX-9 Step 1 で追記）
- cleanup.sql + クラスレベル `@Sql` 方式の規約化（phaseX-9 Step 3 で追記）
- 「`@Transactional` を敢えて付けない」と判断した場合はクラス Javadoc 冒頭に理由を明記（051 派生③で実例：[BatchProductionValidatorContextLoadTest](../../amazia-core/src/test/java/com/example/batch/BatchProductionValidatorContextLoadTest.java)）

**事後検知：**
- 週次 cron `weekly-test-random-order.yml` で `mvn test -Dsurefire.runOrder=random` を実行・失敗時 Issue 自動起票（phaseX-9 Step 5 で導入）
- ローカル push 前の `mvn test -Dsurefire.runOrder=random ×5` を運用化
- Phase 21 で Testcontainers per-class MySQL に移行し、H2 共有 DB そのものを廃止（テーブル間アーカイブ・マルチスレッド検証の構造的分離）
...
> 5. 規約：[docs/ai_context/test_insights.md](test_insights.md) カテゴリ 7-2 を参照
> 6. 「`@Transactional` を付けない」と判断する場合は、その理由をクラス Javadoc 冒頭に明記する

**関連 TPL：** [TPL-009 件数アサーションを伴うテスト追加・改修時](prompt_templates.md#tpl-009-件数アサーションを伴うテスト追加改修時)

**出典：**
- メイン: [docs/troubles/051_ci_apply_scheduled_prices_test_h2_isolation_and_market_e2e_timeout.md](../troubles/051_ci_apply_scheduled_prices_test_h2_isolation_and_market_e2e_timeout.md) 派生①〜③
- 関連: [docs/design/phaseX/phaseX-9_test_isolation_redesign.md](../design/phaseX/phaseX-9_test_isolation_redesign.md), [docs/implementation/phaseX-9_implementation_plan.md](../implementation/phaseX-9_implementation_plan.md)
...
既存 AP-001〜009 に該当しないパターンが 2 件以上累積した時点で、新規 AP として本ファイルに
