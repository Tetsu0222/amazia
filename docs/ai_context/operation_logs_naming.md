...
| お知らせ作成（フェーズ19） | `create_notice` | `console.notice.create` | `POST /api/notices` |
| お知らせ編集（フェーズ19） | `update_notice` | `console.notice.edit`   | `PUT /api/notices/:id` |
| お知らせ削除（フェーズ19） | `delete_notice` | `console.notice.list`   | `DELETE /api/notices/:id` |
| リードタイム更新（フェーズX-5） | `update_shipping_lead_time` | `console.shipping_lead_time.update` | `PATCH /api/shipping-lead-times/:id` |
...
| r3 | 2026-05-10 | phaseX-5（都道府県別リードタイムマスタ化）実装完了に伴い `update_shipping_lead_time` を §6 採番例に追加 |
