...
## ステータス
🟡 実装完了・運用検証残り（2026-05-07）

実装範囲（コード・ドキュメント）はすべて完了。残りはユーザー側で実施する運用作業：
- S3 `schema-snapshots/` prefix のライフサイクル設定（90日 → Glacier、365日 → Expire）
- 故意に DDL 失敗を起こすテストデプロイで改善①②が想定通り検知することを確認
- 本番デプロイ後、Cost Explorer で 1 ヶ月後に追加コスト 0 を確認
...
- [x] `ops/healthcheck/required_tables.txt` が新規作成され、`docs/database_design/README.md` のテーブル一覧と矛盾しないこと
- [x] CLAUDE.md「DB / API 設計書のメンテナンスルール」に主要テーブル定数の同期更新ルールが追記されていること
- [x] deploy.yml に改善①②④の3ステップが追加されていること
- [ ] 故意に DDL 失敗を起こすテストデプロイで改善①が `exit 1` で検知できること（運用検証：ユーザー実施）
- [ ] 故意に WARN を出すテストデプロイで改善②が `::warning::` で検知できること（運用検証：ユーザー実施）
- [ ] 改善④で S3 にスキーマスナップショットが保存されること（90日ライフサイクル設定済）（ライフサイクル設定はユーザー実施）
- [x] `docs/ai_context/operational_insights.md` カテゴリ3 が拡張され、schema.sql 編集時の3点観点が追記されていること
- [x] `docs/ai_context/test_insights.md` カテゴリ7-2 / カテゴリ10 に新観点が追記されていること
- [x] `docs/troubles/README.md` 再発防止アクション表の該当2行（高優先：主要テーブル存在確認 / Core 起動 WARN 抽出）が「✅ X-6 にて対応」に更新されていること
