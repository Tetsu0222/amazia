...
| # | ステップ | 対象 | 内容 |
|---|---------|------|------|
| 1 | DuckDNS ドメイン取得 | 外部サービス | `amazia.duckdns.org` を取得、A レコードを EC2 IP に向ける（暫定） |
| 2 | EC2 オリジンの動作確認 | EC2 / nginx | HTTP のまま `http://amazia.duckdns.org/` で疎通確認 |
| 3 | nginx Host ヘッダ検証追加 | EC2 / nginx | `server_name amazia.duckdns.org;` を設定し直 IP アクセスを遮断 |
| 4 | Vue Router / Vite base path 変更 | Console | Console を `/console/` 配下で動かす対応 |
| 5 | ACM 証明書発行（us-east-1） | AWS | DuckDNS の TXT レコードで DNS 検証 |
| 6 | CloudFront ディストリビューション作成 | AWS | オリジン：EC2 パブリック IP、デフォルト Behavior 設定 |
| 7 | CloudFront Behaviors 設定 | AWS | パスベース 4 ルートの振り分け、Cookie/Authorization 転送 |
| 8 | CNAME（Alternate Domain Name）紐付け | AWS / DuckDNS | CloudFront に `amazia.duckdns.org` を紐付け、DuckDNS の CNAME を CloudFront ドメインへ |
| 9 | アプリ側環境変数の更新 | Core / Console | `PASSWORD_RESET_URL` / `CORS_ALLOWED_ORIGINS` / Cookie Path / Cookie `Secure` |
| 10 | 結合確認・スマホ動作確認 | 全体 | スマホで HTTPS アクセス、ログイン・パスワード再発行・Market 購入の各シナリオ確認 |
| 11 | 設計書・ドキュメント更新 | docs | フェーズ11 §3 を本構成に合わせて更新、トラブル027 として経緯記録 |
...
| Cookie Path（Refresh Token） | `/` | `/console/` |
