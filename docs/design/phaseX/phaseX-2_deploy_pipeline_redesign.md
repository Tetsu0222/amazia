...
## ステータス
✅ 完了（2026-05-03）※③S3転送軽量化のみX-1へ移管

| 改善策 | ステータス |
|--------|-----------|
| ①②セット t3.small変更 + down→pull→up順序 | ✅ 完了 |
| ⑤ インスタンスIDのSecrets化 | ✅ 完了 |
| ヘルスチェックステップ追加 | ✅ 完了 |
| ④ SSMキュー自動検知・リカバリ | ✅ 完了 |
| ③ S3転送軽量化 | 🔲 未着手（X-1へ） |

**計測結果：デプロイ全体 5分未満（①②適用前は約5分超）**
...
- [x] EC2 stop/startなしでデプロイが完結すること
- [x] デプロイ後にSSMキューにInProgressが残らないこと（④実装済み・自動リカバリで対応）
- [x] デプロイ所要時間が現状（約5分）から計測・比較し改善を確認すること（→5分未満を達成）
- [x] インスタンスIDがdeploy.ymlにハードコードされていないこと
