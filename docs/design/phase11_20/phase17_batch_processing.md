...
| 8 | E2E（不整合データを仕込み → 日次バッチ → 通知到達まで・**価格スケジュール E2E を含む**／r7・**Step 8 着手時に Digest Job 本体実装と suppression を抱合**） | 全体 | ✅ 完了（2026-05-09） |
| 9 | `docker compose down -v && docker compose up --build` 完走確認（カテゴリ9） | 全体 | 🟡 静的検証完了（2026-05-09）／実機完走はユーザー実施待ち |
| 10 | ドキュメント更新（README / トラブル記録雛形 / phase11_20 進捗） | docs | ✅ 完了（2026-05-09） |
