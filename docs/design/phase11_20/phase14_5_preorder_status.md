...
## ステータス
✅ 完了（2026-05-07）— Step C-1〜C-4 + Step D すべて完了
...
| r2 | 2026-05-07 | **本フェーズ完了**。Step C-2（PreorderStatusService + JUnit 13 件）/ C-3（GetPreorderStatusController + Console UI 4 カラム拡張 + Console PHPUnit 83/83）/ C-4（Market 表示分岐 + Vitest 73/73）すべてグリーン。Step D で [phase15_delivery_management.md](phase15_delivery_management.md) を r5 に改訂し §3-1 の 6 項目すべてを取り込み完了。実装過程で発生した不具合 #038（products.price/stock NOT NULL 残存）/ #039（Market Checkout preorder モード未対応）/ #040（SKU 価格未登録商品の Market 露出）/ #041（公開判定二重基準）も全て解決済み。最終テスト合計: amazia-core 253/253・amazia-console 83/83・amazia-market 73/73 |
...
## 3. Step D：phase15 r5 への要請整理 ✅ 反映済み（2026-05-07）

[phase15_delivery_management.md](phase15_delivery_management.md) を r5 として改訂し、§3-1 の 6 項目すべてを正式に取り込み完了。phase15 r5 改訂履歴 + 「phase14_5 から取り込んだ要請事項」セクションで反映状況を表にまとめている。
...
- ~~phase15 r5 改訂作業に着手する際は本表を入口として要件を取り込む~~ ✅ 2026-05-07 に取り込み完了
- ~~phase15 r5 完了時に本書 §3 を「✅ 反映済み」として閉じる~~ ✅ 2026-05-07 に閉じた
