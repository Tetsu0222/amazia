...
| 設定値（リードタイム） | 初期は `config/app/Delivery.php` / `application.yml` に定数定義（規約 3-1）。**都道府県別リードタイムのマスタ化は [phaseX-5](../phaseX/phaseX-5_prefecture_based_lead_time.md) で実装済（2026-05-10）✅**（RR-8）。`shipping_lead_times` に該当行が無い場合は本フェーズの全国一律値にフォールバックする |
