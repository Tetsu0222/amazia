...
- 同一トランザクションで `NotificationDispatcher.dispatch('inquiry_alerts', INFO, payload)` を呼び出す。`payload` の組み立てルール（`payload_hash` / `title` / `body`）は §6.1 を参照（新規作成イベントは `payload_hash = SHA-256('inquiry_created:' + inquiries.id)`、`title` / `body` は `config('inquiry.notification_templates').created` から組み立て）。`level=INFO` のため SES メール送出は行われず、`console_notifications` への INSERT のみ実施される（§6.1 後段と整合／RV2-1 対応：r3 で §6.1 への参照に集約）。
...
## 6.1 通知発火点（RV-1 / RV-2 対応：r2 で `title` / `body` 追加・`payload_hash` 修正）

phase17 r6 §5.2 で `console_notifications` の **`title` / `body` は NOT NULL** であるため、本書で文言テンプレートを確定する。文言は `config('inquiry.notification_templates')`（PHP 側）／ `amazia.inquiry.notification-templates`（Spring 側 application.yml）で管理し、ハードコードしない（規約 1-2 / 3-1）。

| イベント | subscription_tag | level | payload_hash 算出元 | title テンプレート | body テンプレート |
|---------|------------------|-------|--------------------|------------------|-----------------|
| Market 顧客が新規問い合わせを作成 | `inquiry_alerts` | `INFO` | `SHA-256('inquiry_created:' + inquiries.id)` | `[問い合わせ] 新規 #{inquiry_id} {subject}` | `{user_name} さんから新規問い合わせが登録されました。\n対象: {target_label}\n件名: {subject}` |
| Market 顧客が既存問い合わせに返信 | `inquiry_alerts` | `INFO` | `SHA-256('inquiry_replied:' + inquiries.id)` ※`messages.id` を含めない（RV-2） | `[問い合わせ] 返信 #{inquiry_id}` | `{user_name} さんが #{inquiry_id} に返信しました。\n件名: {subject}` |
| Console 管理者がステータス変更 | `inquiry_alerts` | `INFO` | `SHA-256('inquiry_status:' + inquiries.id + ':' + new_status)` | `[問い合わせ] ステータス変更 #{inquiry_id}` | `#{inquiry_id} を {old_status} → {new_status} に変更しました。\n件名: {subject}` |
...
`body` テンプレートの `{target_label}` プレースホルダは Service 層で組み立てる：

| `target_type` | `target_label` 値 |
|--------------|------------------|
| `delivery` | `配送 #{deliveries.id}` |
| `product` | `商品 #{products.id} ({products.name})` |
| `sales` | `注文 #{sales.id}` |
| NULL | `（汎用）` |

### dispatcher 呼び出し

phase17 §6.2.2 の `NotificationDispatcher.dispatch(subscription_tag, level, payload)` を呼び出す。`payload` には `title` / `body` / `payload_hash` / `source_job=NULL` / `source_batch_execution_id=NULL` を含める。
...
amazia:
  inquiry:
    ...
    notification-templates:                       # RV-1 対応：r2 で追加
      created:
    ...
    target-labels:                                # RV-1 対応：r2 で追加（target_label 組み立て用）
      delivery: "配送 #{target_id}"
      product:  "商品 #{target_id} ({product_name})"
      sales:    "注文 #{target_id}"
      generic:  "（汎用）"                          # target_type IS NULL の場合
...
| Step | 対象 | 主な作業 | 完了条件 |
|------|------|---------|---------|
| **Step 0** | 前提整備 | (1) `application.yml` / `application-test.properties` / `phpunit.xml` / `docker-compose.yml` に新規環境変数追加。(2) `config/app/Inquiry.php` 新規作成。(3) `config/app.php` への明示読込追加。(4) `routes/api.php` への `Inquiry.php` 明示読込追加。(5) **RV-10 対応**：本書を `docs/design/phase11_20/phase18_inquiry_management.md` として r2 で確定し、Step A 着手前にレビュアー承認を得る。 | `mvn test` / `php artisan test` / `npm run test`（Console / Market）が既存テスト含めて全緑。設計書 r2 がレビュー承認済み |
...
| **Step B** | 機能実装 + API 設計書同期 | (1) Core: `CreateInquiryService` / `ListInquiryService` / `GetInquiryService` / `ReplyInquiryService` / `UpdateInquiryStatusService` / `GetUnreadInquiryCountService` を新規作成。`NotificationDispatcher.dispatch('inquiry_alerts', INFO, ...)` のフック埋め込み。**(1-a) RV-5 対応**：`com.example.inquiry.validator.InquiryTargetOwnershipValidator` を新規作成し、`CreateInquiryService` から呼び出す（`target_type` が `delivery` / `product` / `sales` の場合に所有者検証ロジックを集約。phase11 のロール / phase13 の `market_customers.id` / phase14 の `sales.user_id` / phase15 の `deliveries → sales` 結合を 1 ファイルに閉じ込める）。(2) Console SPA: `Bell.vue` / `InquiryList.vue` / `InquiryDetail.vue` 新規 + `useVisibilityPolling` Composable 新規（RV-8）。Header.vue にベルマーク埋め込み。(3) Market SPA: `MyPageInquiryList.tsx` / `MyPageInquiryDetail.tsx` / `MyPageInquiryNew.tsx` 新規。マイページメニューに「問い合わせ」追加。(4) Console / Market の Pass-through ルート追加。(5) operation_logs 記録の埋め込み。**(6) RV-10 対応 — API 設計書同期**：`docs/api_design/Core_API.md` に `/api/console/inquiries/*` 系・`/api/customer/inquiries/*` 系を追記、`docs/api_design/Console_API.md` に Pass-through 経路を追記、`docs/api_design/Market_API.md` に React 側呼び出しを追記。 | Market から問い合わせ作成 → Console ベルマーク件数増加 → 管理者が返信 → Market 側で返信表示、の end-to-end が本番 HTTPS 構成（CloudFront → EC2）で通る。Core / Console / Market 全テスト緑。**`docs/api_design/` 配下が実装と完全一致** |
...
- 通知 dispatch（`NotificationDispatcher`）がモック呼び出し検証で正しく走る（新規作成は `payload_hash = SHA-256('inquiry_created:'+id)`、顧客返信は `payload_hash = SHA-256('inquiry_replied:'+id)`（RV-2：`messages.id` を含めない）、ステータス変更は `SHA-256('inquiry_status:'+id+':'+new_status)` と一致）
- 通知 dispatch 時に `console_notifications.title` / `body` が `config('inquiry.notification_templates')` のテンプレートから組み立てられて NOT NULL で INSERT される（RV-1）
...
phase17 が先に r6 完了 → 本書実装着手という順序になった場合でも、本書スコープ（ベルマーク 30 秒ポーリング + 通知 INSERT）は phase17 §6.2 の `NotificationDispatcher` のみに依存し、Job 追加要請には依存しないため、本書実装は単独で完結する。
...
- フォルダ構成（規約 2-1）：
  ...
  - **Vue Composable（RV-8 対応：r2 で追加）**：`resources/vue/composables/useVisibilityPolling.ts`（規約 2-3 Shared 思想）。phase19 以降の集約 Bell でも再利用予定
