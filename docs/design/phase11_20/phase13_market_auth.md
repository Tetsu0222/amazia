...
## 改訂履歴
- 2026-05-06 完了：本番動作確認済み・E2E（フロント側 vitest シナリオ）追加・README / phase13 ステータスを完了に反映
...
| # | ステップ | 対象 | ステータス |
|---|---------|------|-----------|
| 0 | 設計書 X-3 反映・Redis→DB 直に改訂 | docs | ✅ |
| 1 | DB マイグレーション V5（`market_customers` 等 5 テーブル） | core | ✅ |
| 2 | JPA Entity / Repository 実装 | core | ✅ |
| 3 | セッション認証 Filter（自作 + CSRF） | core | ✅ |
| 4 | 顧客 API 実装（登録 / ログイン / マイページ / パスリセ） | core | ✅ |
| 5 | 郵便局 CSV 取込バッチ + 検索 API | core | ✅（バッチ組込みはフェーズ17・実行手順は §14.1） |
...
| 9 | E2E（登録 → ログイン → マイページ → ログアウト → パスリセ） | 全体 | ✅（`amazia-market/src/test/auth_flow.e2e.test.jsx`） |
| 10 | ドキュメント更新（README / トラブル記録 / phase11_20 進捗） | docs | ✅ |

## 14.1 ステップ5 郵便番号 CSV 取込の実行手順

`ImportPostalCsvRunner` は `--import-postal-csv` フラグ起動時のみ Service を実行する `ApplicationRunner`。フェーズ17 で `@Scheduled` 化するまでは、初回投入・再投入とも以下の手動コマンドで実行する。

### 14.1.1 ローカル

ローカルでは `docker-compose.local.yml`（`build:` 定義と必須環境変数を持つ）を併用する。
本番 `docker-compose.yml` 単体だと `image: ECR/...` 参照で `build` 定義がなく、ローカル変更が反映できないため。

```powershell
# 1. MySQL のみ起動（既に起動済なら不要）
docker compose -f docker-compose.yml -f docker-compose.local.yml up -d mysql

# 2. amazia-core をローカルビルド（pom.xml の finalName 変更を反映）
docker compose -f docker-compose.yml -f docker-compose.local.yml build amazia-core

# 3. 取込実行（Web サーバは立ち上げず、使い捨て JVM で実行）
docker compose -f docker-compose.yml -f docker-compose.local.yml run --rm --no-deps `
  amazia-core java -jar app.jar --import-postal-csv
```

実行後、`postal_addresses` 件数が KEN_ALL.CSV の件数（約 12 万件）程度になっていれば成功。

### 14.1.2 本番 EC2（SSM Run Command 経由）

既存の `amazia-core` コンテナは Web サーバとして稼働中のため、`docker compose exec` で同 jar を再起動するとポート 8080 競合で即死する。
また、`ImportPostalCsvRunner` は ApplicationRunner として起動するが、Spring Boot は CommandLineRunner 完了後も Web サーバ運用のため main thread を終了しない。
そのため `docker compose run` を foreground で叩くとコンテナがハングする。

対策として以下を組み合わせる：
1. Web サーバを `stop` してメモリを空ける（t3.micro は `-Xmx384m × 2` 同時稼働で OOM 寸前のため）
2. `run -d` で取込コンテナをデタッチ起動・名前固定
3. 取込完了マーカー（`[postal-csv-runner] import done`）をログから検知
4. 検知後にコンテナ停止・削除 → Web サーバ復帰

SSM Run Command（`AWS-RunShellScript`）に投入するコマンド：

```bash
cd /home/ssm-user/amazia && \
sudo docker compose stop amazia-core && \
sudo docker compose run --rm --no-deps -d --name postal-import amazia-core java -jar app.jar --import-postal-csv && \
( timeout 600 bash -c 'until sudo docker logs postal-import 2>&1 | grep -q "\[postal-csv-runner\] import done"; do sleep 5; done' ) && \
sudo docker logs postal-import 2>&1 | grep -E "\[postal-csv\] import (success|failed)" && \
sudo docker stop postal-import && \
sudo docker rm postal-import 2>/dev/null; \
sudo docker compose start amazia-core
```
...
aws ssm send-command \
  ...
  --comment "phase13 step5 KEN_ALL.CSV initial import" \
  --parameters commands="$(cat <<'CMD'
cd /home/ssm-user/amazia && \
sudo docker compose stop amazia-core && \
sudo docker compose run --rm --no-deps -d --name postal-import amazia-core java -jar app.jar --import-postal-csv && \
( timeout 600 bash -c 'until sudo docker logs postal-import 2>&1 | grep -q "\[postal-csv-runner\] import done"; do sleep 5; done' ) && \
...
- SSM Run Command の Status が `Success`
- 出力に `[postal-csv] import success attempt=1 saved=NNNNN elapsedMs=...` を含むこと
- 続けて `aws ssm send-command` で件数確認：
  ```bash
  sudo docker exec amazia-mysql mysql -uamazia -pamazia_pass amazia -e "SELECT COUNT(*) FROM postal_addresses;"
  ```
  → 10 万件以上（KEN_ALL.CSV 2026-05 時点で 124,822 件想定）
- `curl -s http://localhost:8080/api/customer/postal-addresses?postal_code=1000001` で東京都千代田区千代田が返ること
- Web サーバ復帰確認：`sudo docker compose ps amazia-core` で `Up` 状態

### 14.1.4 注意事項

- SSM 出力は 24,000 文字でカット。Hibernate の INSERT ログは膨大なので、コマンドは `grep` で成功/失敗ラインのみ抽出する設計にしてある
- `timeout 600` は完了マーカー監視のループ用（KEN_ALL.CSV ダウンロード遅延も含めた上限）
- 取込中は Web サーバ停止 = Market 顧客 API ダウン。ポートフォリオ用途のため許容
