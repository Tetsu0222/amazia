...
`shipping_lead_times` マスタは `shipping_methods` × `address.prefecture` のリードタイム日数を保持し、`DeliveryScheduleService.calculate(...)` から参照される。マスタ未登録／prefecture 厳密不一致／NULL の場合は `application.properties` の `amazia.delivery.lead-time-days.*` 全国一律値にフォールバックする。
...
| コントローラー | `ListShippingLeadTimeController` |

**クエリパラメータ（任意）**

| パラメータ | 型 | 説明 |
|------------|-----|------|
| shippingMethodId | long | 配送方法 ID でフィルタ（例: `?shippingMethodId=1` で 47 都道府県分） |

**レスポンス**: `ShippingLeadTimeResponse` の配列（id / shippingMethodId / prefecture / leadTimeDays / updatedAt）。クエリ無しなら全 141 件。
...
| コントローラー | `GetShippingLeadTimeController` |
...
| メソッド | PATCH |
| パス | `/api/shipping-lead-times/{id}` |
| コントローラー | `UpdateShippingLeadTimeController` |
| サービス | `UpdateShippingLeadTimeService.update` |

**ヘッダ**

| 名前 | 必須 | 説明 |
|------|------|------|
| X-User-Id | ○ | 操作者 ID（operation_logs 記録用） |

**リクエストボディ**

| パラメータ | 型 | 必須 | 説明 |
|------------|-----|------|------|
| leadTimeDays | int | ○ | リードタイム日数。0 以上（0 は無効化扱い）。負数は 400/422 |

**レスポンス**: `ShippingLeadTimeResponse`（更新後）

**operation_logs**: `action='update_shipping_lead_time' / screen_name='console.shipping_lead_time.update' / api_name='PATCH /api/shipping-lead-times/:id'`

> POST / DELETE は提供しない。新都道府県発生時は `schema.sql` の `INSERT IGNORE` で追加する設計（設計書 §機能詳細）。
