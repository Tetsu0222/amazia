...
## ステータス
✅ 解決済（2026-05-06）— 本番 HTTPS で主要動作（ログイン / Cookie / 画像 / Market / Console）すべて確認
...
### Console 経由の SKU 画像配信が auth.jwt 配下にあり 401（[033](033_console_image_file_route_under_auth_jwt.md)）
`<img src>` は Authorization ヘッダを運ばないのに、Console の画像配信ルートが `auth.jwt` ミドルウェアの中にあり必ず 401。Market と同じ思想で公開ルート（auth 外）に移動して解消。

これら 4 件は X-3 が直接の原因ではないが、本番動作確認をしたのが X-3 完了後だったため、X-3 のデプロイで踏み抜いた形。031・032・033 として独立起票しつつ、ここからもクロスリンクして経緯を追えるようにする。

---

## 将来課題（X-3 範囲外として記録）

X-3 動作確認の過程で気付いたが、X-3 の必須スコープではないため別タイミングで対応する事項。

### 1. Spring コンテナの非 root 化と storage 所有権統一
- 現状 `amazia-core` は Dockerfile に `USER` 指定がなく **root として起動**している
- 結果、bind mount した `/home/ssm-user/amazia-storage/Product/images/sku/4` が `root:root` 所有になっている
- 動作上は問題ないが、将来 Spring イメージを非 root 化（典型的なセキュリティベストプラクティス）した際に書き込み拒否となる
- 対策案：
  - amazia-core/Dockerfile に `RUN addgroup -g 1000 app && adduser -u 1000 -G app -D app && chown -R app:app /app` を追加
  - `USER 1000:1000` を追加
  - deploy.yml の `chown -R 1000:1000 /home/ssm-user/amazia-storage` をその UID と整合
  - 既存ファイルの所有権マイグレーション手順を併せて準備
- 優先度：低（学習用途では許容範囲）

### 2. パスワード再発行の本番動作確認
- SES がサンドボックスから出ているか、送信先メールアドレスが検証済みか未確認
- `admin@amazia.example.com` はダミードメインなので、検証済みの実メールを持つテストユーザーで確認する必要がある

### 3. スマホ Safari / 各ブラウザでの実機確認
- Cookie SameSite の挙動はブラウザ実装で差が出ることがある
- iOS Safari で `Secure / HttpOnly / SameSite=Lax` の Cookie が期待通りに保持されるかの実機検証

### 4. 自動リフレッシュの実時間動作確認
- `JWT_ACCESS_TTL=900` 秒経過後の自動リフレッシュが本番でも機能するかの 15 分超え動作確認
- 仕組み自体は authApi.js の 401 ハンドラで実装済みのため、潜在不具合の可能性は低い

### 5. CloudFront のキャッシュポリシー再評価
- 静的ファイル（Vue 成果物）と動的レスポンスのキャッシュ挙動を本番運用で観察し、無料枠 1TB/月 の使用状況に応じてポリシー調整
- Behavior 単位の TTL を `CachingOptimized` から個別調整する余地
