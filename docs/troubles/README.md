...
| 052 | [weekly random-order CI 失敗：`InventoryInconsistencyToNotificationE2ETest` の admin ロール参照が空](052_weekly_random_order_inventory_e2e_role_lookup_failure.md) | phaseX-9 で稼働開始した週次 random 順序 CI が `persistAdminWithSubscription:151` の `roleRepository.findByCode("admin").orElseThrow()` で `NoSuchElement: No value present` を検知。本テストは `BatchAlertNotifier` の `REQUIRES_NEW` 貫通検証目的でクラス `@Transactional` を意図的に外した 6 クラスのうちの 1 つ（051 派生③で整理済み）。共有 H2 + context 切替 + random 順序の組合せで `roles` マスタが一瞬空になるエッジケースに、テストヘルパー側の自衛が無かった。`findByCode` を `orElseGet` で「無ければ作る」フォールバックに切替。AP-009 のサブパターンとして「`REQUIRES_NEW` 検証で `@Transactional` を外したテストのマスタデータ参照は防御を持つ」観点を追加候補 | ✅ 解決済 | - | - |
| 053 | [`DigestNotificationDispatchJob` の `@Scheduled` 初期 tick とテスト fixture のロック競合](053_digest_scheduled_initial_tick_lock_contention.md) | 052 調査中のローカル `mvn test` 全件実行で `DigestRestartCatchUpE2ETest.E2E_7:79` が `verify(sesClient, atLeastOnce()).sendEmail(...)` で `Wanted but not invoked` を検知。`DigestNotificationDispatchJob` は他 12 ジョブと違い `@ConditionalOnProperty(scheduler-enabled, true)` ガードを持たず（phase17 設計書 M-2 / J-1 で `BATCH_DIGEST_ENABLED` を `BATCH_SCHEDULER_ENABLED` から独立にする意図的設計）、`@Scheduled(fixedRate)` の **initialDelay 未指定**で context 起動完了直後に initial tick が発火し、テスト本体の手動 `digestJob.run` と `BatchJobLockRegistry.tryAcquire` を競合 → テスト本体が「skip: another instance is running」で弾かれて SES が一度も呼ばれない。**設計書 M-2 / J-1 を壊さない方針として(B) 本番コード `@Scheduled` に `initialDelayString` を fixedRate 1 周期相当で追加 + (C) テスト側で `digest-interval-ms` を 24h にオーバーライドの二重防御**を実施。phase17 Step 10 で予約していた雛形 `XXX_batch_lock_leak.md` を本 NNN にリネームし、原因が雛形の想定（AOP proxy / OOM / JVM クラッシュ）と異なっていたことも併記 | ✅ 解決済 | - | - |
| 054 | [フェーズ18 inquiries / inquiry_messages が本番 DB に作成されない（FK 型不一致 / 044 同型）](054_inquiries_table_missing_fk_type_mismatch.md) | フェーズ18 デプロイ後、phaseX-6 の post-deploy ヘルスチェック「主要テーブル存在確認」で `inquiries` / `inquiry_messages` が「46 件中 44 件しか存在しない」として exit 1。原因は `schema.sql` フェーズ18 §1-1 の `inquiries.user_id` を `BIGINT NOT NULL` で書いたが、参照先の `market_customers.id` が `BIGINT UNSIGNED`（phase13 / Laravel 由来）で型不一致 → MySQL FK 作成失敗 → `spring.sql.init.continue-on-error=true` が WARN 化して握り潰し、テーブル自体が作成されず。H2 では型差を吸収するためローカルテストではすり抜けた。**`inquiries.user_id` を `BIGINT UNSIGNED NOT NULL` に修正**して再デプロイで解決。phase15 ER §備考に「Laravel 由来 ID 系は `BIGINT_UNSIGNED`、Core オリジナルは `BIGINT`、FK では型を合わせる必要がある（029 / 037 起因）」の警告が r1 から存在していたが、フェーズ18 r1 設計時に見落とした。phaseX-6 の `required_tables.txt` 防御線が本不具合を 1 デプロイ目で検知できた点で、044 教訓の効果を再確認 | ✅ 解決済 | - | - |
