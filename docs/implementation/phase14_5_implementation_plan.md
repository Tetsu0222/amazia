...
- [x] Core 起動時に schema.sql の冪等 SQL がエラーなく流れる（2026-05-07 不具合 #038 / #041 対応で Core を `docker compose up --build amazia-core` で再起動した際に確認済み。`ALTER TABLE ... ADD COLUMN` の重複は `spring.sql.init.continue-on-error=true` で吸収される）
...
## 4. Step C-4：Market 表示分岐 + Vitest ✅ 完了（2026-05-07）

### 完了サマリ
- Core: `ProductMarketSummary` / `ProductMarketDetail` に `preorderStatus` + 4 カラム（releaseDate / preorderStartDate / acceptPreorder / acceptBackorder）を追加
- Core: `ListProductMarketService` を `PreorderStatusService` 経由の判定に切替
  - `NOT_PUBLIC` のみ一覧から除外（旧仕様の「在庫 0 除外」を撤廃 = §4-2 に整合）
  - `minPrice` の `Integer.MAX_VALUE` プレースホルダバグも合わせて解消（price 未登録 SKU を除外して min 取得）
- Core: `SkuAggregateControllerTest` を §4-2 仕様に更新、SOLD_OUT 表示・NOT_PUBLIC 除外の 2 件を追加（4 → 6 件、最終 251/251 グリーン）
- Market: `preorderStatus.js`（新規）でステータス→ラベル/Chip色/ボタン文言/ボタン挙動 のメタデータを集約
- Market: `ProductList.jsx` に Chip ラベル + ステータス別補足（在庫数 / 発売日 / 予約開始日 / 在庫切れ予約受付中）を追加
- Market: `ProductDetail.jsx` に Chip ラベル・発売日／予約開始日表示・購入ボタン分岐（購入する / 予約する / 非活性 / 非表示）を追加。`PRE_ORDER` / `BACK_ORDER` のときは checkout に `&preorder=1` を付加
- Market Vitest:
  - `ProductList.test.jsx`: ON_SALE / PRE_ORDER / PRE_ORDER_NOT_STARTED / BACK_ORDER / SOLD_OUT の 5 ステータスを検証（5 → 10 件）
  - `ProductDetail.test.jsx`: PRE_ORDER（ボタン活性 + 発売日）/ NOT_STARTED（非活性）/ BACK_ORDER / SOLD_OUT（ボタン非表示）/ 予約フロー遷移の 5 件追加（11 → 15 件）
- 設計書: [Core_API.md](../api_design/Core_API.md) / [Market_API.md](../api_design/Market_API.md) の Market 集約 API 節を更新（preorderStatus + 4 カラム反映、旧「在庫 0 除外」記述を訂正）
- 全層テスト: amazia-core 251/251、amazia-market vitest 63/63、amazia-console phpunit 83/83（C-3 から変化なし）
...
## 5. Step D：phase15 r5 への要請整理 ✅ 完了（2026-05-07）

### 完了サマリ
- [phase15_delivery_management.md](../design/phase11_20/phase15_delivery_management.md) を **r4 → r5** に改訂
- 新規セクション「**phase14_5 から取り込んだ要請事項**」を追加し、6 項目（P5-1〜P5-6）の反映先と対応状況を表でマッピング
  - P5-1: `shipping_methods` マスタの INSERT IGNORE マイグレーション仕様を追記
  - P5-2: OrderConfirmationService からの `DeliveryCreationService.createForSales` フック呼び出しを実コード差分付きで明示
  - P5-3: 出荷時（PENDING→SHIPPED）の予約購入 SKU 在庫減算ロジックを新規セクションで定義
  - P5-4: 出荷時在庫不足時の「例外＋PENDING 維持」挙動を明文化（`ResponseStatusException(CONFLICT)` + `operation_logs.shipping_blocked_insufficient_stock`）
  - P5-5: `RegisterInboundService` 新設と既存 `ReceiveProductSkuStockService` 流用方針を確定（責務分離 4 ステップ）
  - P5-6: 配送ステータス CANCELED / DELIVERY_FAILED / RESCHEDULED は r5 でもスコープ外確認のみ（既存 §本フェーズのスコープ外 で対応）
- [phase14_5_preorder_status.md](../design/phase11_20/phase14_5_preorder_status.md) §3 を「✅ 反映済み」として閉じ、改訂履歴に r2 を追加し全体ステータスを「✅ 完了」に変更
...
### 5-2. Step D 完了条件
- [x] phase15 r5 設計書（または要件リスト）に 6 項目すべて取り込み済み
- [x] phase14_5_preorder_status.md §3 を「✅ 反映済み」として閉じる
...
- [x] Step C-1 〜 C-4 / Step D すべて完了
- [x] Core / Console / Market 全層テストグリーン（amazia-core 253/253・amazia-console 83/83・amazia-market 73/73）
- [x] Console 商品登録 UI から 4 カラムを登録でき、Market で 6 種類のステータスが表示分岐される
- [x] phase15 r5 設計書に §3-1 の要件が取り込まれている
- [x] `docs/design/phase11_20/phase14_5_preorder_status.md` のステータスを `🟡 進行中` → `✅ 完了（2026-05-07）` に更新

### 実装過程で発生した不具合（全て解決済み）
- [#038](../troubles/038_products_price_stock_not_null_drift.md): products.price/stock の NOT NULL 残存
- [#039](../troubles/039_market_checkout_preorder_mode_missing.md): Market Checkout の preorder モード未対応
- [#040](../troubles/040_market_lists_products_without_sku_price.md): SKU 価格未登録商品の Market 露出
- [#041](../troubles/041_publish_judgement_dual_basis_secs_vs_jst_zero.md): 公開判定の二重基準
