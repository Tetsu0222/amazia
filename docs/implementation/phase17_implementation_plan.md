...
| 18 | `BATCH_NOTIFICATIONS_SUBSCRIPTION_TAGS`（**Step 6-4 で追加**） | ✅ | ✅ | ✅ |

> 2026-05-08 Step 1.5 時点で 17 個 × 3 箇所すべて反映済み（§2-6-1）。
> 2026-05-08 Step 6-4 で `BATCH_NOTIFICATIONS_SUBSCRIPTION_TAGS` を追加し 18 個に増加（§8-4 の r9 改訂と同期）。
...
## 6. Step 5 — フォルトインジェクション ✅ 完了（2026-05-08）

実装ファイル：
- `amazia-core/src/main/java/com/example/faultinjection/service/FaultInjectionLogger.java`（5-1）
- `amazia-core/src/main/java/com/example/faultinjection/service/SalesMismatchInjector.java`（5-2 / `@Profile("!production")`）
- `amazia-core/src/main/java/com/example/faultinjection/service/InventoryMismatchInjector.java`（5-3 / `@Profile("!production")` / H-7）
- `amazia-core/src/main/java/com/example/faultinjection/service/DeliveryTroubleInjector.java`（5-4 / `@Profile("!production")` / R-2 / H-7）
- `amazia-core/src/main/java/com/example/batch/job/TriggerFaultInjectionJob.java`（5-5 / `@Profile("!production")` / M-4）
- `amazia-core/src/main/java/com/example/batch/service/RandomGeneratorAdapter.java` に `nextIntBetween(int, int)` 追加
- `amazia-core/src/main/java/com/example/batch/job/SalesReconciliationJob.java` に `SalesMismatchInjector` の Optional 注入を追加（MISMATCH 検出時に履歴記録）

完了条件の検証：
- 本番プロファイルで Injector / `TriggerFaultInjectionJob` の Bean 不在（`FaultInjectionProfileGuardTest`）
- `fault_injection_logs.environment='production'` の INSERT が CHECK で拒否されること（同上）
- 補償 SKU TX が `[fault_injection][...]` 接頭辞で記録されること（`InventoryMismatchInjectorTest` / `DeliveryTroubleInjectorTest`）
- Trigger 経由で 3 Injector が一括発火（`TriggerFaultInjectionJobTest`）
...
## 7. Step 5.5 — 価格スケジュール Core API + Console Pass-through ✅ 完了（2026-05-08）

実装ファイル：

Core（amazia-core）：
- `sku/repository/ProductSkuPriceRepository.java` — `findFirstBySkuIdAndIsActiveTrue` / `findBySkuIdOrderByStartDateDescIdDesc` 追加、`deactivateActive` を `clearAutomatically=true, flushAutomatically=true` に強化
- `sku/service/GetProductSkuPriceService.java` — 改修（5.5-1a）
- `sku/service/CreateProductSkuPriceService.java` — 改修（5.5-1b）
- `sku/service/ListSkuPriceHistoryService.java` — 新設（5.5-1f）
- `sku/controller/ListSkuPriceHistoryController.java` — 新設
- `scheduledprice/service/GetScheduledSkuPriceService.java` — 新設（5.5-1c）
- `scheduledprice/service/RegisterScheduledSkuPriceService.java` — 新設（5.5-1d）
- `scheduledprice/service/DeleteScheduledSkuPriceService.java` — 新設（5.5-1e）
- `scheduledprice/controller/{Get,Register,Delete}ScheduledSkuPriceController.java` — 新設

Console（amazia-console）：
- `app/ScheduledPrice/Service/{Get,Register,Delete}ScheduledSkuPriceService.php` — 新設
- `app/ScheduledPrice/Controller/{Get,Register,Delete}ScheduledSkuPriceController.php` — 新設
- `app/Sku/Service/ListSkuPriceHistoryService.php` — 新設
- `app/Sku/Controller/ListSkuPriceHistoryController.php` — 新設
- `routes/api/Sku.php` — 4 本のルーティングを追記（`/skus/{id}/prices/history` / `/skus/{id}/scheduled-price` × GET / PUT / DELETE）

テスト（Core JUnit 26 / Console PHPUnit 8 すべて緑）：
- Core：`GetProductSkuPriceServiceIsActiveTest` / `CreateProductSkuPriceServiceIsActiveTest` / `ListSkuPriceHistoryServiceTest` / `GetScheduledSkuPriceServiceTest` / `RegisterScheduledSkuPriceServiceTest` / `DeleteScheduledSkuPriceServiceTest`
- Console：`tests/Feature/ScheduledPrice/ScheduledPriceTest.php`（取得 / UPSERT / 削除 / 履歴 / バリデーション / Core 422 透過）
...
- 一覧 API のレスポンス形式：`{"items":[...], "total":N, "offset":M, "size":S}`
  - r9 修正（2026-05-08）：当初 r8 の表記は `page/size` だったが、実装は `LIMIT/OFFSET` 方式が扱いやすく `offset/size` で統一。`docs/api_design/Core_API.md` を本実装に合わせ、本書もここで揃える
...
- 操作ログ：`action='trigger_batch_manual'` / `target_type='batch_jobs'` / `screen_name='ConsoleBatchManagementPage'` / `api_name='POST /api/console/batch/{job_name}/run'`
  - 実装は `TriggerBatchManualService`（Core）が `OperationLog` を `REQUIRES_NEW` で書き込む
  - 503 / 404 / 400 ケースでは記録しない（操作前に弾かれるため）
  - `X-User-Id` 欠落は 400（`operation_logs.user_id NOT NULL` を満たすため必須化）

### 8-4. Step 6-4: phase11 への要請事項のフック実装（13.0 / N-9 / K-3）

phase11 の `CreateUserService` / `UpdateUserService` から `SyncNotificationSubscriptionsService.applyForUserRole(userId, roleCode)` を呼び出す：
- 自動購読対象ロール（`amazia.batch.notifications.auto-subscribe-roles`：既定 `admin / senior_admin / eternal_advisor`）に該当 → 全タグへ UPSERT（`email_enabled=true / in_app_enabled=true`）
- 非対象ロール（admin → user 降格を含む） → 当該ユーザの全行を `email_enabled=false / in_app_enabled=false`（行は物理削除しない）

**タグ一覧の集約場所（r9 改訂・2026-05-08）：** 当初 r8 では `config/notifications.php`（Console 側 PHP config）と書かれていたが、フックの呼び出し元は Core 側 `CreateUserService` / `UpdateUserService` であり、Core から PHP config は参照できない。実装は `application.properties` の `amazia.batch.notifications.subscription-tags`（CSV）に集約し、`SyncNotificationSubscriptionsService` が `@Value` 経由で取得する（規約 4-1 / 1-2）。`auto-subscribe-roles` と同居させて単一情報源とする。

### 8-5. テスト
- Console PHPUnit：通知センター / 履歴 / 手動起動の権限チェック・既読遷移
- Core JUnit：
  - `SyncNotificationSubscriptionsServiceTest`：admin/senior_admin/eternal_advisor で全タグ UPSERT、user で何もしない、admin → user 降格で全行 FALSE 化、二重実行で冪等
  - `UserControllerTest`：`POST /api/users` で admin 登録時に5タグ自動購読、user 登録時は0件、`PUT` で昇格・降格フローを確認
  - `BatchManualTriggerControllerTest`：手動起動成功時に `operation_logs` が記録されること、404/400 では記録されないこと

### 8-6. 完了条件（Step 6）
- 3 画面の vitest / 手動 E2E が緑
- 操作ログが正しい命名で記録される
- 設計書（`docs/api_design/Core_API.md` / `Console_API.md`）にバッチ系 5 API + 価格スケジュール 6 API が反映されている
...
### 11-0. Step 8 着手前に判明したスコープ拡張

着手前の調査で、Step 7 の `BatchAlertNotifier` Javadoc に「Step 8 / J-3 / R-10 の本実装で被せる前提」と明記されていた **`DigestNotificationDispatchJob` 本体と suppression ロジックが未実装**であることが発覚。
パッケージ構成図（§1-2）には `DigestNotificationDispatchJob` が記載されているが、Step 3 〜 Step 7 のいずれの作業範囲にも本実装が含まれていなかった。
E2E-6 / E2E-7 はダイジェスト機能が存在することを前提とする以上、Step 8 のサブステップとして以下を組み込む形で実施した：

- Step 8-0：`BatchAlertNotifier.dispatch` に suppression 判定（`(sourceJob, payloadHash)` 一致 + `suppression-minutes` 以内なら `suppressed=true` で persist して SES をスキップ）を追加
- Step 8-1：`DigestNotificationDispatchJob`（5 分間隔・`BATCH_DIGEST_ENABLED` で ON/OFF・タグ単位集計）+ `DigestDispatchService`（テンプレ展開と `digest_sent_at` UPDATE を REQUIRES_NEW で担う）の新設

`AbstractBatchJob` の field 注入と `@Transactional` AOP proxy が衝突するため、ジョブクラス側に直接 `@Transactional` を持たせず、`DigestDispatchService` に切り出した（[user memory: phaseX9_concession_inventory] と整合）。
...
| # | シナリオ | 前提 | 期待 | テストファイル |
|---|---------|------|------|------|
| E2E-1 | 不整合 → 通知到達 | `inventories.quantity` を直接 +1 | inventory_alerts 購読者の Inbox に SES + 通知センター未読 +1 | `InventoryInconsistencyToNotificationE2ETest` |
| E2E-2 | フォルト確率の統計検証 | `RandomGeneratorAdapter` モック差し替え（R-14 採用） | `random.nextDouble() < rate` のときだけ発火・履歴も対応分のみ残る | `FaultInjectionRateStatisticsE2ETest` |
| E2E-3 | `BATCH_ENABLED=false` alias | scheduler/manual=false ＋ digest=true（独立性検証） | 業務 Bean は不在・Digest Bean は継続（J-1） | `BatchEnabledFlagMatrixE2ETest$E2E_3_BatchEnabledFalseAlias` |
| E2E-4 | 業務 OFF + 手動 ON | `scheduler-enabled=false` ＋ `manual-trigger-enabled=true` | 業務 Bean 不在・OnDemandJob マップに `TriggerFaultInjectionJob` は残る | `BatchEnabledFlagMatrixE2ETest$E2E_4_SchedulerOffManualOn` |
| E2E-5 | 手動 OFF / フラグ戻し | `manual-trigger-enabled=false`（ネスト 2 ApplicationContext で 503 / 200 を確認） | OFF で 503・ON で 200 / Bean は同一でフラグ起因の 503 だけが消える | `BatchEnabledFlagMatrixE2ETest$E2E_5_ManualOff` / `$E2E_5_ManualOnRestoresImmediately` |
| E2E-6 | 業務 OFF + ダイジェスト ON | `scheduler-enabled=false` ＋ `digest-enabled=true` | 業務 Bean 不在・Digest Bean 継続 | `BatchEnabledFlagMatrixE2ETest$E2E_6_SchedulerOffDigestOn` |
| E2E-7 | ダイジェスト OFF → ON 復帰でまとめ送信 | OFF 期間 = `suppressed=true / digest_sent_at IS NULL / 60 分以前` 行 3 件を直接 INSERT → ジョブ実行 | 1 通の SES（件名にカウント数 3）・対象行の `digest_sent_at` が全件埋まる（K-4 / N-7） | `DigestRestartCatchUpE2ETest` |
| E2E-8 | 本番 TriggerFaultInjectionJob | `@ActiveProfiles({"test", "production"})` で起動 | Bean マップに不在・`POST /api/console/batch/TriggerFaultInjectionJob/run` が 404（M-4） | `TriggerFaultInjectionProductionE2ETest` |
| E2E-9 | 価格スケジュール（r7） | `apply_date = TODAY` 登録 / `apply_date = TOMORROW` 登録の 2 ケース | TODAY は当日のジョブで切替・履歴に旧価格 / TOMORROW は据え置き（`is_pending=TRUE`）。「翌日待機」は時計操作不要で同等担保 | `ScheduledPriceLifecycleE2ETest` |

合計 13 テストすべて緑（Step 8 全体）。`com.example.batch.e2e` パッケージに新規配置。phaseX-9 規約（cleanup.sql + クラスレベル `@Sql(BEFORE_TEST_METHOD)` + 自テスト ID フィルタ前後差分）を全件遵守。

### 11-2. 副次変更

- **`BatchAlertNotifierSesIntegrationTest`：** suppression 動作の検証として BAN_8 / BAN_9 を追加。BAN_8 は「直近 60 分以内に同 `(sourceJob, payload_hash)` があれば `suppressed=true` で残し SES をスキップ」、BAN_9 は「無ければ通常通り SES 送信し `suppressed=false` で残す」を Mockito でリフレクション注入で検証。
- **`amazia-core/src/test/resources/cleanup/e2e_full.sql` 新設：** E2E は `batch_executions / console_notifications / notification_subscriptions / operation_logs / fault_injection_logs` を横断するため、E2E 専用の cleanup を整備。

### 11-3. 完了条件
- [x] 全 9 シナリオ（13 テスト）が緑
- [x] 確率テスト（E2E-2）は `RandomGeneratorAdapter` モック化（R-14 採用）でフレーキー化を回避
- [x] phase17 全 534 テストが緑（リグレッションなし）
- [x] `DigestNotificationDispatchJob` + `DigestDispatchService` 新設（設計書 §6.4.2 完全準拠）
- [x] `BatchAlertNotifier` に suppression 判定を被せ、設計書 §6.4.1 の重複抑制を実装
