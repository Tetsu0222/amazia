...
### 1-6. Step 0-5: ベーススキーマ作成（**Core Flyway マイグレーション**）

業務テーブルは既存方針に合わせて **Core 側 Flyway** で管理する（Console Laravel migrations には業務テーブルを追加しない）。

**新規ファイル**: `amazia-core/src/main/resources/db/migration/V6__phase14_base_tables.sql`
...
### 1-8. Step 0-7: 完了確認
- Core を起動して Flyway V6 が成功（`mvn spring-boot:run` または docker-compose）
...
| # | 論点 | 確定方針 |
|---|------|---------|
| 1 | DB マイグレーションの管理元 | ✅ **Core 側 Flyway**（V6〜）。Console Laravel migrations には業務テーブルを追加しない（既存方針：業務テーブルは Core に集約） |
